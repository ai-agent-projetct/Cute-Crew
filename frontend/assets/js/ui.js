...
const NAV = [
  { href: '/', label: 'Home' },
  { href: '/shop.html?gender=boys', label: 'Boys' },
  { href: '/shop.html?gender=girls', label: 'Girls' },
  ...
  <div class="header-blur fixed top-0 left-0 right-0 z-50 border-b border-line">
    <div class="max-w-7xl mx-auto px-4">
      <div class="flex items-center gap-4 h-20">
        ...
        <form action="/shop.html" class="flex-1 max-w-xl hidden sm:block" role="search">
          <div class="relative">
            <input name="search" type="search" placeholder="Search for tees, dresses, rompers…" class="field !py-2.5 !pl-10 !rounded-full" autocomplete="off">
        ...
        <div class="flex items-center gap-1 ml-auto">
          ${user
            ? `<button id="btn-logout" class="btn-ghost px-4 py-2 text-sm hidden md:block">Hi, ${String(user.name).split(' ')[0]} · Logout</button>`
            : `<button id="btn-login" class="btn-ghost px-4 py-2 text-sm hidden md:block">Login</button>`}
      ...
      <nav class="flex gap-6 md:justify-center overflow-x-auto -mb-px text-sm" aria-label="Categories">
        ${NAV.map((n) => `<a href="${n.href}" class="nav-link ${page === n.href.split('?')[0] && (page !== '/' || n.href === '/') && (n.href === '/' ? page === '/' : location.search === (n.href.split('?')[1] ? '?' + n.href.split('?')[1] : location.search)) ? 'active' : ''} ${n.hot ? '!text-gold' : ''}">${n.label}</a>`).join('')}
      </nav>
    </div>
  </div>
  <div class="h-[7.75rem]"></div>`;
}
